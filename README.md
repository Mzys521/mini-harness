...
![Status](https://img.shields.io/badge/Status-Phase%205%20%28RAG%29%20Done-brightgreen)
...
mini-harness 是一个分阶段演进的 LLM Agent 框架（Harness），目标是从内核出发，逐步补全一个生产级 Agent 系统所需的全部基础设施。项目按 11 个阶段推进，**Phase 1–5 已完成（v0.5.0），Phase 6（MCP）待启动**。

已完成的 Phase 1–5 把「模型会调用工具」这件事拆解成四个可独立演进的子系统：

- **工具运行时**：工具注册与 Schema 导出、Pydantic 参数校验、权限检查、超时控制、重试策略与中间件（如日志）
- **上下文工程**：Token 预算管理、基于策略的滑动窗口裁剪、工作状态（WorkingState）注入、检索结果（RAG）注入、超长工具结果截断
- **状态与持久化**：Run / Step / Checkpoint 状态模型、SQLite 仓储（Repository）+ 工作单元（UnitOfWork）事务、检查点快照支持崩溃恢复
- **检索增强（RAG）**：文档加载与字符切分（带重叠）、稳定 ID、向量化（Qwen / OpenAI 嵌入）、向量存储（Chroma / 内存）、稠密检索与可插拔重排、证据化结果投影
...
| `harness/context` | P3 | TokenBudget 预算、ContextPolicy 策略、ContextBuilder 组装与裁剪、滑动窗口；P5 起支持检索上下文注入 |
...
| `harness/retrieval` | P5 | 文档加载 / 字符切分 / 稳定 ID / 嵌入（Qwen·OpenAI）/ 向量存储（Chroma·内存）/ 稠密检索 / 重排 / 证据投影 / recall@k |
| `app_tools` | P2·P5 | 示例工具：计算器、sleep、delete_file（P2）；`search_knowledge_base` 知识检索（P5，多租户过滤） |
| `scripts` | P5 | 知识导入与检索演示脚本（`ingest_knowledge` / `search_knowledge`） |
...
- Python 3.12+
- 运行依赖：`openai`、`pydantic`、`python-dotenv`、`chromadb`
...
示例会注册计算器与知识检索等工具，向模型发起一道多步计算题，由 Agent 自主完成多轮工具调用，并通过日志中间件打印每次调用的耗时与状态。

### 知识导入与检索

`scripts/` 下提供两个演示脚本（运行前请先修改脚本内的文档路径）：

```bash
# 加载与切分演示：读取文本文件并打印切分结果
python -m scripts.ingest_knowledge

# 端到端检索演示：向量化 → 存入向量库 → 检索并打印证据
...
- **检索链路可插拔**：嵌入（Qwen / OpenAI）、向量存储（Chroma / 内存）、重排器均为 Protocol 接口，检索结果以证据格式（来源 / 章节 / 分数）注入上下文
